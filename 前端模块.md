这个前端页面的样子和session方案那个差不多，因此就不记录页面搭建的过程了，重点记录不同的地方。

# axios请求封装

为了统一，get请求一律携带请求参数，post请求一律携带json格式的请求体数据
...
没登录情况下，修改访问路径就可以直接访问到主页，这样肯定不行的，登录功能如同虚设
这里需要配置下路由守卫，实现以下功能：
1、如果路由的目的地址的名字是welcome开头（想去欢迎页面）的，且已登录，那么强制跳转到主页，不能回到欢迎页面
2、如果路由的目的地址的路径是home开头（想去主页）的，但未登录，那么强制跳转到登录页面
3、正常情况下，放行

src/net/index.js
axios请求中再封装一个判断用户是否授权的方法，并且暴露出去
```js
//判断用户是否未授权
...
需求：将登录，注册，重置密码这三个页面的切换变得丝滑一点

只需要在< router-view/>之间添加element-plus提供的< transition/>即可
...
原先src/pages/WelcomeIndex.vue的背景颜色写死了，适配不了，需要变为动态适配
```css
.right {
  width: 450px;
  text-align: center;
